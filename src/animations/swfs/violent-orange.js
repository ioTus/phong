/**
 * Violent Orange — Generative duplication animation.
 */
import { MovieClip } from '../../lib/phong-runtime.js';

export async function setup(stage) {
  stage.bgColor = '#000000';
  const shapeImg = await stage.loadSVG('/extracted/swfs/violent-orange/shapes/1.svg');
  ...
  container.onFrame = function(self) {
    i++;
    if (i <= 120) {
      const clone = template.duplicateMovieClip('cir' + i, i);
      if (clone) {
        clone._visible = true;
