/**
 * Wave Fractal Waver — Recursive branching fractal tree.
 */
import { MovieClip } from '../../lib/phong-runtime.js';

export async function setup(stage) {
  stage.bgColor = '#0a0e1a';
  const branchImg = await stage.loadSVG('/extracted/fractal/wave-fractal-waver/shapes/1.svg');
  let depth = 0, levels = 14;
  const MAX_DEPTH = 128;
        ...
        if (levels >= 0 && depth < MAX_DEPTH) {
          ...
          for (let n = 0; n < splitNum; n++) {
            ...
            if (child) {
              ...
              child._y = -100;
              child._xscale = levels > 0 ? 100 - (100 / 14) * (14 - levels) : 50;
