/**
 * So Sea Sick 11 — Generative duplication animation.
 */
import { MovieClip } from '../../lib/phong-runtime.js';

export async function setup(stage) {
  stage.bgColor = '#0a0e1a';
  const shapeImg = await stage.loadSVG('/extracted/graph/so-sea-sick-11/shapes/1.svg');
  ...
  container.onFrame = function(self) {
    i++;
    if (i <= 48) {
      const clone = template.duplicateMovieClip('O' + i, i);
      if (clone) {
        clone._visible = true;
        clone._x = (i - 48/2) * (384 / 48);
        clone._y = Math.sin(i * 0.2) * 50;
        const s = Math.sin(i * 0.15) * 200 + 100;
        clone._xscale = s;
        clone._yscale = s;
        clone._rotation = i * 3;
        clone._alpha = 100 - (i / 48) * 60;
