/**
 * Intertrip3 — Generative duplication animation.
 */
import { MovieClip } from '../../lib/phong-runtime.js';

export async function setup(stage) {
  stage.bgColor = '#000000';
  const shapeImg = await stage.loadSVG('/extracted/!random/intertrip3/shapes/1.svg');
  ...
  container.onFrame = function(self) {
    i++;
    if (i <= 16) {
      const clone = template.duplicateMovieClip('Ball' + i, i);
      if (clone) {
        clone._visible = true;
        clone._x = (i - 16/2) * (128 / 16);
        clone._y = Math.sin(i * 0.2) * 50;
        const s = Math.sin(i * 0.15) * 200 + 100;
        clone._xscale = s;
        clone._yscale = s;
        clone._rotation = i * 3;
        clone._alpha = 100 - (i / 16) * 60;
